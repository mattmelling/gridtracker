...
function loadAlerts()
{
	...
	else
	{
		...
		{
			if ( g_alerts[key].type != 0 && g_alerts[key].type != 2 && g_alerts[key].type != 4 && g_alerts[key].type != 5 && g_alerts[key].type != 6)
				delete g_alerts[key];
			if ( g_alerts[key].repeat == 3 )
				delete g_alerts[key];
		}
...
function checkAlerts( DEcallsign,  grid,  originalMessage, callsignRecord, band, mode)
{
	var hadAlert = false;
	for (var key in g_alerts )
	{
		...
		if ( nalert.type == 0 )   // callsign exatch match
		{
		...
		else if ( nalert.type == 6 )   // callsign regex
		{
			try
			{
...
function handleAlert( nAlert, target, lastMessage , callsignRecord, grid)
{
	...
	if ( nAlert.type == 0 || nAlert.type == 5 ||  nAlert.type == 6)
	{
		...
		if ( nAlert.notify == 1 )
			speakAlertString( "Callsign", target, null );
		if ( nAlert.notify == 2 )
			displayAlertPopUp( "Seeking", target, null );
	}
	...
	if ( nAlert.type == 2 )
	{
		if ( nAlert.notify == 0 )
			playAlertMediaFile( nAlert.filename );
		if ( nAlert.notify == 1 )
			speakAlertString( "Grid square", grid, null );
		if ( nAlert.notify == 2 )
	...
	if ( Object.keys(g_alerts).length > 0 )
	{
		...
		for (var key in g_alerts )
		{
			if ( g_alerts[key].needAck )
			{
				...
				worker += "<td style='color:cyan'>" + (g_alerts[key].lastMessage.length>0?g_alerts[key].lastMessage:"-") + "</td>";
				ageString = userTimeString( g_alerts[key].lastTime*1000);
				worker += "<td>" + (g_alerts[key].lastTime>0? ageString:"-") + "</td>";
				worker += "</tr>";
			}
		}
		worker += "</table>";
...
{
	if ( g_alertFlasher )
		clearTimeout(g_alertFlasher);

	alertPopListDiv.innerHTML = "<font color='red'><h2>Gathering Alerts<h2></font>";
...
function alertTypeChanged()
{
	...
	{
		alertValueSelect.innerHTML = '<input id="alertValueInput" disabled="true" type="text" class="inputTextValue" value="'+ myDEcall +'" maxlength="12"  size="5" oninput="ValidateCallsign(this,null);" / >';
		ValidateCallsign(alertValueInput,null);
	}
...
g_alertValueOptions["0"]="<img title='Audio File' style='margin:-1px;margin-bottom:-4px;padding:0px' src='/img/icon_audio_16.png'>";
...
function displayAlerts()
{
	...
	if ( Object.keys(g_alerts).length > 0 )
	{
		...
		for (var key in g_alerts )
		{
			...
			ageString = userTimeString( g_alerts[key].lastTime*1000);
			worker += "<td>" + (g_alerts[key].lastTime>0? ageString:"-") + "</td>";
			worker += "<td style='cursor:pointer' onclick='resetAlert(\"" + key + "\")'><img src='/img/reset_24x48.png' style='height:17px;margin:-1px;margin-bottom:-3px;padding:0px' ></td>";
			worker += "<td style='cursor:pointer' onclick='deleteAlert(\"" + key + "\")'><img src='/img/trash_24x48.png' style='height:17px;margin:-1px;margin-bottom:-3px;padding:0px'></td>";
			worker += "</tr>";
		}
		worker += "</table>";
		worker += "</div>";
...
function processClassicAlerts()
{
	...
	{
		if (  document.getElementById(key).checked == true && g_classic_alert_counts[key] > 0 )
		{
